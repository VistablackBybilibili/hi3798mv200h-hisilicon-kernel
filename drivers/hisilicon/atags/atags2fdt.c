...
#define ATAG_SMCU_PARAM 0x48694E7e
static u32 smcu_mem_addr = 0;

#ifdef CONFIG_BLK_DEV_RAM
extern int parse_initmrd_bootargs(char *bootargs);
#endif

#ifdef CONFIG_SUPPORT_BOOTLOG_RSVD
#define ATAG_BOOTLOG_PARAM 0x48694E7F
extern u32 bootlog_mem_addr;
extern u32 bootlog_mem_size;
#endif
...
int parse_nodev_bootargs(void *fdt, char *p)
{
	...
	if (strstr(cmd_line, " nouart3 ")) {
		node = fdt_path_offset(fdt, "uart3");
		if (node >= 0) {
			fdt_setprop_string(fdt, node, "status", "disabled");
		}
	}
		      ...
		      int (*parse)(const struct tag *, void *))
{
	...
	for_each_tag(atag, atag_list) {
		...
		} else if(atag->hdr.tag == ATAG_SMCU_PARAM) {
			smcu_mem_addr = *((u32 *)(&atag->u));
#ifdef CONFIG_SUPPORT_BOOTLOG_RSVD
		} else if(atag->hdr.tag == ATAG_BOOTLOG_PARAM) {
			bootlog_mem_addr = *((u32 *)(&atag->u));
			bootlog_mem_size = *((u32 *)(&atag->u) + 1);
			if(bootlog_mem_addr && bootlog_mem_size){
				BUG_ON(fdt_add_memory_reserve((u64)bootlog_mem_addr,
					(u64)bootlog_mem_size));
			} else {
				printk("Warning: no bootlog mem/size reserved.");
			}
#endif
		} else
			parse(atag, fdt);
...
#ifdef CONFIG_MACH_HI3716MV430
	BUG_ON(fdt_add_memory_reserve((u64)smcu_mem_addr,
...
#ifdef CONFIG_ARM64
	atag_list = early_memremap((resource_size_t)0x100, 4096);
#else
	atag_list = __va(0x00000100);
#endif
...
u32 get_smcu_mem_phyaddr(void)
{
	return smcu_mem_addr;
}
EXPORT_SYMBOL(get_smcu_mem_phyaddr);
