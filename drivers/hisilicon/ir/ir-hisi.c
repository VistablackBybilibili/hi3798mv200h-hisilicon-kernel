...
static int hix5hd2_ir_init(struct hix5hd2_ir_priv *priv)
{
	int timeout = 10000;
	u32 val, rate;

	writel(0x01, priv->base + IR_ENABLE);
	while (readl(priv->base + IR_BUSY)) {
		if (timeout--) {
			udelay(1);
		} else {
...
static irqreturn_t hix5hd2_ir_rx_interrupt(int irq, void *data)
{
	...
	if (irq_sr & INTMS_OVERFLOW) {
		...
		writel(INT_CLR_OVERFLOW, priv->base + IR_INTC);
	...
	if ((irq_sr & INTMS_SYMBRCV) || (irq_sr & INTMS_TIMEOUT)) {
		...
		if (irq_sr & INTMS_SYMBRCV)
			writel(INT_CLR_RCV, priv->base + IR_INTC);
		if (irq_sr & INTMS_TIMEOUT)
			writel(INT_CLR_TIMEOUT, priv->base + IR_INTC);
	}
...
static int hix5hd2_ir_probe(struct platform_device *pdev)
{
	...
	struct hix5hd2_ir_priv *priv = NULL;
...
MODULE_DESCRIPTION("IR controller driver");
MODULE_AUTHOR("xxx");
