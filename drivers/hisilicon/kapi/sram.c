...
{
	if (sram_ctrl)
		gen_pool_free(sram_ctrl->pool, (unsigned long)(uintptr_t)addr, len);
}
...
#if defined(CONFIG_PM) && !defined(CONFIG_ARCH_S40) && !defined(CONFIG_ARCH_HI3716MV420N) && !defined(CONFIG_ARCH_HI3798MX)
...
static int __init sram_init(void)
{
	...
	sram_ctrl->viraddr = ioremap((unsigned long)(uintptr_t)sram_ctrl->phyaddr,
				     sram_ctrl->size);
	if (!sram_ctrl->viraddr)
		goto out2;

	status = gen_pool_add_virt(sram_ctrl->pool,
				   (unsigned long)(uintptr_t)sram_ctrl->viraddr,
				   (unsigned long)(uintptr_t)sram_ctrl->phyaddr,
				   sram_ctrl->size, -1);
	if (status < 0) {
