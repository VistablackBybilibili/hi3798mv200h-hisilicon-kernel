 ...
 *             SecOutputSW  provides internal implementation for printf family
 *             API, such as sprintf, swprintf_s.
 * Author: lishunda
 * Create: 2014-02-25
 * Notes: see: http://www.cplusplus.com/reference/cstdio/printf/
 ********************************************************************************
 */
...
#ifdef SECUREC_USE_SPECIAL_DIV64
#include <asm/div64.h>
#endif
...
#define SECUREC_FMT_TABLE_SIZE     (337)
...
#define SECUREC_MUL_TEN_ADD_BEYOND_MAX(val) (((val) > SECUREC_INT_MAX_DIV_TEN))
...
/* call system sprintf to format float value */
#define SECUREC_INDIRECT_SPRINTF(pTextLen, formatAttr, formatBuf, fFmtStr, tmp) do { \
    if ((formatAttr).dynWidth && (formatAttr).dynPrecision) { \
        *(pTextLen) = SecIndirectSprintf((formatBuf).str, (char *)(fFmtStr), (formatAttr).fldWidth, formatAttr.precision, (tmp)); \
    } else if ((formatAttr).dynWidth) { \
        *(pTextLen) = SecIndirectSprintf((formatBuf).str, (char *)(fFmtStr), (formatAttr).fldWidth, (tmp)); \
    } else if ((formatAttr).dynPrecision) { \
        *(pTextLen) = SecIndirectSprintf((formatBuf).str, (char *)(fFmtStr), (formatAttr).precision, (tmp)); \
    } else { \
        *(pTextLen) = SecIndirectSprintf((formatBuf).str, (char *)(fFmtStr), (tmp)); \
    } \
} SECUREC_WHILE_ZERO
...
#define SECUREC_SPECIAL_DWORD(val32, numBase) \
            case numBase: \
                do { \
...
#define SECUREC_SPECIAL_QWORD(val64, numBase) \
            case numBase: \
                do { \
                    *--formatBuf.str = digits[do_div((val64), (numBase))]; \
                } while ((val64) != 0)
#else
#define SECUREC_SPECIAL_QWORD(val64, numBase) \
            case numBase: \
...
#define SECUREC_SAFE_WRITE_PREFIX(src, txtLen, outStream, outChars) do { \
            for (ii = 0; ii < txtLen; ++ii) { \
                *((SecChar *)(void *)(outStream->cur)) = *(src); \
...
#define SECUREC_SAFE_WRITE_STR(src, txtLen, outStream, outChars) do { \
            if (txtLen < 12 /* for mobile number length */) { \
                ...
                (void)memcpy(outStream->cur, src, ((size_t)(unsigned int)txtLen * (sizeof(SecChar)))); \
                outStream->cur += (size_t)(unsigned int)txtLen * (sizeof(SecChar)); \
            } \
            outStream->count -= txtLen * (int)(sizeof(SecChar)); \
            *(outChars) = *(outChars) + (txtLen); \
        } SECUREC_WHILE_ZERO
...
static int SecDecodeTypeC(SecFormatAttr *attr, unsigned int cValue, SecFormatBuf *formatBuf, SecBuffer *buffer)
{
    ...
    if (attr->flags & SECUREC_FLAG_SHORT) {
        ...
        char tmpChar[SECUREC_PREFIX_LEN];
        wchar_t wChar = (wchar_t)cValue;
        tmpChar[0] = (char)(wChar & 0x00ff);
        tmpChar[1] = '\0';
...
#endif
{
    ...
    static const unsigned char fmtCharTable[SECUREC_FMT_TABLE_SIZE] = {
...
NORMAL_CHAR:
            ...
            case STAT_WIDTH:
                ...
                if (ch == SECUREC_CHAR('*')) {
                    ...
                    if (formatAttr.fldWidth < 0) {
                        ...
                        if (formatAttr.fldWidth > SECUREC_INT_MAX_DIV_TEN) {
                            return -1;
                        }
                    }
            ...
            case STAT_PRECIS:
                ...
                if (ch == SECUREC_CHAR('*')) {
                    ...
                    if (formatAttr.precision > SECUREC_INT_MAX_DIV_TEN) {
                        return -1;
                    }
                    ...
                    formatAttr.precision =
                        (int)SECUREC_MUL_TEN((unsigned int)formatAttr.precision) +
                            (unsigned char)(ch - SECUREC_CHAR('0'));
                    formatAttr.dynPrecision = 0;
                }
                break;
                    ...
                    case SECUREC_CHAR('a'):
#if SECUREC_ENABLE_SPRINTF_FLOAT
                        {
                            ...
                            if (formatAttr.flags & SECUREC_FLAG_LONG_DOUBLE) {
                                if (formatAttr.precision > (SECUREC_INT_MAX_DIV_TEN - SECUREC_FLOAT_BUFSIZE_LB)) {
                                    noOutput = 1;
                                    break;
                                }
                                bufferSize = SECUREC_FLOAT_BUFSIZE_LB + formatAttr.precision + 1;
                            } else {
                                if (formatAttr.precision > (SECUREC_INT_MAX_DIV_TEN - SECUREC_FLOAT_BUFSIZE)) {
                                    noOutput = 1;
                                    break;
                                }
                                bufferSize = SECUREC_FLOAT_BUFSIZE + formatAttr.precision + 1;
                            }
                            if (formatAttr.fldWidth > bufferSize) {
                                bufferSize = formatAttr.fldWidth + 1;
                            }
                            ...
                            if (bufferSize >= SECUREC_BUFFER_SIZE) {
                                 ...
                                 */
                                /* size include '+' and '\0' */
                                bufferSize += SECUREC_FLOAT_BUF_EXT;
...

                            {
                                ...
                                const SecChar *pFloatFmt = format - 2;    /* point to the position before 'f' or 'g' */
                                int k;
                                int fFmtStrLen;
                                ...
                                if (formatAttr.flags & SECUREC_FLAG_LONG_DOUBLE) {
                                    ...
                                    SECUREC_INDIRECT_SPRINTF(&textLen, formatAttr, formatBuf, fFmtStr, tmp);
                                } else
#endif
                                {
        ...
        /* not linux vxwoks */
                        ...
                        formatAttr.precision = 1;
#else
                        formatAttr.precision = 2 * sizeof(void *);  /* precision of different systems */
#endif
...
OUTPUT_INT:
                        {
                             ...
                             * if 'p',add 0x prefix,else not add prefix 
                             */
                            if (number == 0) {
                            ...
                            if (number > 0) {
                                ...
                                switch (radix) {
                                    ...
                                    SECUREC_SPECIAL_QWORD(number, SECUREC_RADIX_DECIMAL);
                                        break;
                                ...
                                if (number <= 0xFFFFFFFFUL) {
                                    ...
                                    do {
                                        if (SecU64Div32((SecUnsignedInt32)((number >> SECUREC_OFFSET_BITS_WORD) >> \
                                                         SECUREC_OFFSET_BITS_WORD),
                                                         (SecUnsignedInt32)number,
                                                         (SecUnsignedInt32)radix, &quotientHigh,
                                                         &quotientLow, &digit) != 0) {
                                            noOutput = 1;
                                            break;
                                        }
                                        *--formatBuf.str = digits[digit];
                                        ...
                                        number = (number << SECUREC_OFFSET_BITS_DWORD) + quotientLow;
                                    } while (number != 0);
#else
                                    switch (radix) {
                                        /* the compiler will optimize div 10 */
