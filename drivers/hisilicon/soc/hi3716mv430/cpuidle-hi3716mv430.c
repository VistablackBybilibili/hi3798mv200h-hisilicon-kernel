...
extern void flash_l1_cache_hi3798mv2x(void);
extern void cpu_powerup_hi3798mv2x(void);
extern void cpu_powerdown_hi3798mv2x(void);
...
static struct cpumask cpuidle_cpu_mask = CPU_MASK_ALL;
#else
...
static u32 __iomem *cpu_resume_vaddr = NULL;
/******************************************************************************/
...
static void __maybe_unused pmc_dump_stat(int cpuid)
{
	...
	u32 tdown;
	u32 tup;
	struct cpu_pm_stat_t *pmstat = NULL;
...
static void pmc_powerdown(int cpuid)
{
	...
	if (cpuid == 0) {
		pmcaddr = __io_address(REG_PERI_PMC0);
		regval = readl(pmcaddr);
		regval |= (1 << 3) | (1 << 6) | (1 << 7) | (1 << 8) | (1 << 9);
	} else if (cpuid == 1) {
		pmcaddr = __io_address(REG_PERI_PMC76);
		regval = readl(pmcaddr);
		regval |= (1 << 3) | (1 << 6) | (1 << 7) | (1 << 8) | (1 << 9);	
	} else if (cpuid == 2) {
		pmcaddr = __io_address(REG_PERI_PMC77);
		regval = readl(pmcaddr);
		regval |= (1 << 3) | (1 << 6) | (1 << 7) | (1 << 8) | (1 << 9);	
	} else if (cpuid == 3) {
		pmcaddr = __io_address(REG_PERI_PMC78);
		regval = readl(pmcaddr);
		regval |= (1 << 3) | (1 << 6) | (1 << 7) | (1 << 8) | (1 << 9);
	} else {
		BUG();
...
static int notrace cpu_entry_powerdown(unsigned long arg)
{
	...
	//flash_l1_cache_hi3798mv2x();
	v7_exit_coherency_flush(louis);

	cpu_powerdown_hi3798mv2x();
...
static int hi3798mv2xx_cpuidle_entry(struct cpuidle_device *dev,
				     struct cpuidle_driver *drv, int idx)
{
	...
	//}
#ifdef CONFIG_TEE
	cpu_resume_vaddr[0] = (u32)virt_to_phys(cpu_powerup_hi3798mv2x);
#else
	cpu_resume_vaddr[0] = 0xe51ff004;
	cpu_resume_vaddr[1] = (u32)virt_to_phys(cpu_powerup_hi3798mv2x);
#endif
	isb();
	dsb();
...
static int __init hi3798mv2xx_cpuidle_init(void)
{
	...
	hi3798mv2xx_cpuidle_driver.states[1].exit_latency = latency;
	hi3798mv2xx_cpuidle_driver.states[1].target_residency = residency;

	return cpuidle_register(&hi3798mv2xx_cpuidle_driver, NULL);
}

device_initcall(hi3798mv2xx_cpuidle_init);
