...
#ifdef SATA_PIN_MUX
	//SATA PWR_EN PIN MUX
	regval = hisata_read(priv->peri_ctrl_base + 0x109c);
	regval&=~0x7;
	regval|=0x6;
	hisata_write(regval, priv->peri_ctrl_base + 0x109c);
	//SATA LED PIN MUX
	regval = hisata_read(priv->peri_ctrl_base + 0x10A0);
	regval&=~0x7;
	regval|=0x6;
	hisata_write(regval, priv->peri_ctrl_base + 0x10A0);
#endif
