 ...
 *
 * Create By Lai Yingjun 2017.4.7
 *
******************************************************************************/

#define DRVNAME    "hiahci-phy0"
...
#define PERI_COMBPHY0_STATE        0x85c
...
static u32 nano_reg_read(struct hiahci_phy_priv *priv, u32 nano_testread_addr)
{
	...
	tmp_val = hisata_read(priv->peri_ctrl_base + PERI_COMBPHY0_CFG);
	tmp_val &= ~NANO_TEST_WRITE;
	tmp_val &= ~NANO_TEST_ADDR;
	tmp_val |= (nano_testread_addr << 12);
	hisata_write(tmp_val, priv->peri_ctrl_base + PERI_COMBPHY0_CFG);

	tmp_val = hisata_read(priv->peri_ctrl_base + PERI_COMBPHY0_STATE);
...
static int hiahci_phy_init(struct phy *phy)
{
	...
	mux = hisata_read(priv->peri_ctrl_base + PERI_CTRL);
...
#ifdef SATA_PIN_MUX
	//SATA PWR_EN PIN MUX
	regval = hisata_read(priv->peri_ctrl_base + 0x10A4);
	regval&=~0x7;
	regval|=0x6;
	hisata_write(regval, priv->peri_ctrl_base + 0x10A4);
	//SATA LED PIN MUX
	regval = hisata_read(priv->peri_ctrl_base + 0x10A8);
	regval&=~0x7;
	regval|=0x6;
	hisata_write(regval, priv->peri_ctrl_base + 0x10A8);
#endif
...
static int hiahci_phy_power_on(struct phy *phy)
{
	...
	/*SSC offset */
	nano_reg_write(priv,0x3,0x2);

	/*release SATA controller reset*/
	hisata_write(0xf, priv->peri_ctrl_base + 0x2400);
	...
	if (!(regval&(0xf<<8))) {
		...
		nano_reg_write(priv,0x7,0x2);
	}

	regval = hisata_read(priv->iobase+HI_SATA_PORT0_OFF + 0x28);
	if (!(regval&(0xf<<8))) {
		/* 6G_IDLE_DET_TRIM=2'b11' */
		nano_reg_write(priv,0x7,0x3);
	}
...
static struct phy_ops hiahci_phy0_ops = {
...
static int hiahci_phy_probe(struct platform_device *pdev)
{
	...
	struct hiahci_phy_priv *priv = NULL;
	...
	phy = devm_phy_create(&pdev->dev, NULL, &hiahci_phy0_ops);
...
static const struct of_device_id hiahci_phy0_of_match[] = {
	{.compatible = "hiahci-phy0",},
	{ },
};
MODULE_DEVICE_TABLE(of, hiahci_phy0_of_match);

static struct platform_driver hiahci_phy0_driver = {
	.probe  = hiahci_phy_probe,
	.driver = {
		.name = DRVNAME,
		.of_match_table = hiahci_phy0_of_match,
	}
};
/*****************************************************************************/

static int __init hiahci_phy0_module_init(void)
{
	pr_info("registered new sata phy driver\n");
	return platform_driver_register(&hiahci_phy0_driver);
}
module_init(hiahci_phy0_module_init);
/*****************************************************************************/

static void __exit hiahci_phy0_module_exit(void)
{
	platform_driver_unregister(&hiahci_phy0_driver);
}
module_exit(hiahci_phy0_module_exit);

MODULE_AUTHOR("Lai Yingjun");
MODULE_DESCRIPTION("Hisilicon Hi3796Mv200 SATA PHY driver");
