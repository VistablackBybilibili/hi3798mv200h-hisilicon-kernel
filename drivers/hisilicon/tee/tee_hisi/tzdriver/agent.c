...
int TC_NS_set_nativeCA_hash(unsigned long arg)
{
	...
	mb_pack->operation.params[0].value.b = (unsigned int)((virt_to_phys(buftotee) >> 16) >> 16); /*lint !e572*/
	mb_pack->operation.params[1].value.a = buflen;

	mb_pack->uuid[0] = 1;
	smc_cmd.uuid_phys = virt_to_phys(mb_pack->uuid);
	smc_cmd.uuid_h_phys = (virt_to_phys(mb_pack->uuid) >> 16) >> 16; /*lint !e572*/
...
{
	struct __smc_event_data *event_data = NULL;
	struct __smc_event_data  *tmp_data = NULL;
	unsigned long flags;
			 ...
			 TC_NS_Shared_MEM *shared_mem)
{
	...
	mb_pack->operation.params[0].value.b = (virt_to_phys(shared_mem->kernel_addr) >> 16) >> 16; /*lint !e572*/
...
int TC_NS_unregister_agent(unsigned int agent_id)
{
	...
	mb_pack->operation.params[0].value.b = (virt_to_phys(event_data->buffer->kernel_addr) >> 16) >> 16; /*lint !e572*/
