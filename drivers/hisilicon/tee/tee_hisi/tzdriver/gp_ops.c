				   ...
				   uint8_t kernel_api)
{
	...
	if (kernel_api && ((unsigned long)(uintptr_t)src > TASK_SIZE)) {
#else
				 ...
				 uint8_t kernel_api)
{
	...
	/* Source is kernel valid address */
#ifdef CONFIG_DEVCHIP_PLATFORM
	if (kernel_api && ((unsigned long)(uintptr_t)dest > TASK_SIZE)) {
#else
					...
					uint8_t flags)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (teec_tmpmem_type(param_type, 2)) {
			...
			operation->buffer_h_addr[i] = (virt_to_phys((void *)temp_buf) >> 16) >> 16;
			...
			list_for_each_entry(shared_mem,
				...
				if (shared_mem->user_addr ==
					...
					if (((shared_mem->len - client_param->memref.offset) >= buffer_size)
						...
						void *buffer_addr = (void *)(uintptr_t)((unsigned long)(uintptr_t)shared_mem->kernel_addr
						...
						operation->buffer_h_addr[i] = (virt_to_phys(buffer_addr) >> 16) >> 16; /*lint !e572*/
...
#else
			tloge("in %s err: don't support ion!\n", __func__);
			ret = -EFAULT;
			break;
#endif
		} else {
	...
	for (i = 0; i < 4; i++) {
		...
		if (teec_tmpmem_type(param_type, 1)) {
			...
			if (!operation->sharemem[i]->from_mailbox && operation->mb_buffer[i]
				...
				void *buffer_addr = (void *)((uintptr_t)operation->sharemem[i]->kernel_addr
					+ client_param->memref.offset);
	...
	for (i = 0; i < 4; i++) {
		...
		if ((TEEC_MEMREF_TEMP_INPUT == param_type) ||
			...
			if (virt_addr_valid(temp_buf) && /*lint !e648 */
			    !ZERO_OR_NULL_PTR((uintptr_t)temp_buf))
		   ...
		   TC_NS_DEV_File *dev_file, uint8_t flags)
{
	...
	if (sizeof(uint32_t) == dev_file->pub_key_len &&
		...
		smc_cmd->login_data_h_addr = (virt_to_phys(mb_pack->login_data) >> 16) >> 16; /*lint !e572*/
