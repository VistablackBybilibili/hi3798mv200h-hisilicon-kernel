 ...
 *        Created:  05/01/2015
 *
 *         Author:  Zang Leigang (sdk), zangleigang@hisilicon.com
					...
					bool reliable)
{
	...
	ioc->write_flag = (uint32_t)is_write | (uint32_t)(((uint32_t)is_write & (uint32_t)reliable) << 31);
	ioc->opcode =
			...
			uint32_t reliable)
{
	...
	ret = memset_s(status_frame, sizeof(*status_frame), 0, sizeof(*status_frame));
	if (ret != 0) {
...
mmc_blk_ioctl_rpmb_copy_data(struct mmc_blk_ioc_rpmb_data *rdata)
{
	...
	for (i = 0; i < MMC_IOC_MAX_RPMB_CMD; i++) {
		...
		err = memcpy_s(&idata->data[i].ic, sizeof(struct mmc_ioc_cmd), &rdata->data[i].ic,
		       sizeof(struct mmc_ioc_cmd));
		if (err != 0) {
			pr_err("memcpy ic failed\n");
		}
		err = memcpy_s(idata->data[i].buf, idata->data[i].buf_bytes, rdata->data[i].buf,
		       idata->data[i].buf_bytes);
		if (err != 0) {
			pr_err("memcpy buff failed\n");
		}
	}
			   ...
			   struct mmc_blk_ioc_rpmb_data *rdata)
{
	...
	if ((md == NULL) || (!((unsigned int)md->area_type & MMC_BLK_DATA_AREA_RPMB))) {
		err = -EINVAL;
		return err;
	}
	...
	for (i = 0; i < MMC_IOC_MAX_RPMB_CMD; i++) {
		...
		err = mmc_set_blockcount(card, data.blocks,
					 (unsigned int)curr_cmd->write_flag & (1 << 31));
		if (err)
			goto cmd_rel_host;
		...
		err = memcpy_s(curr_cmd->response, sizeof(cmd.resp), cmd.resp, sizeof(cmd.resp));
		if (err != 0) {
			pr_err("memcpy response failed\n");
		}
		if (!curr_cmd->write_flag) {
			err = memcpy_s(rdata->data[i].buf, curr_data->buf_bytes, curr_data->buf,
			       curr_data->buf_bytes);
			if (err != 0) {
				pr_err("memcpy buf failed\n");
			}
		}
