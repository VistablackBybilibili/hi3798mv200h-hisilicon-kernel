	...
	for (i = 0; i < MAILBOX_PAGE_MAX; i++) {
		if (m_zone.pages[i].count) {
			pr_info("page[%02d], order=%02d, count=%d, page=%pK\n",
				...
				m_zone.pages[i].page);
			used += (1<<(unsigned int)m_zone.pages[i].order);
		}
	}
...
static void mailbox_show_details(void)
{
	...
	for (i = 0; i < MAILBOX_PAGE_MAX; i++) {
		...
		if (m_zone.pages[i].count) {
			left = 1 << (unsigned int)m_zone.pages[i].order;
...
{
	unsigned int i;
	struct page *page = virt_to_page((uintptr_t)ptr); /*lint !e648*/
...
static unsigned int mb_dbg_add_entry(void *ptr)
{
	...
	ret = memset_s(new_entry, sizeof(*new_entry), 0, sizeof(*new_entry));
	if (ret != 0) {
		tloge("memset entry failed\n");
	}
...
static int mailbox_register(const void *mb_pool, unsigned int size)
{
	...
	operation.params[0].value.b = (virt_to_phys(mb_pool) >> 16) >> 16;
	operation.params[1].value.a = size;

	uuid[0] = 1;
	smc_cmd.uuid_phys = virt_to_phys(uuid);
	smc_cmd.uuid_h_phys = (virt_to_phys(uuid) >> 16) >> 16;
...
int mailbox_mempool_init(void)
{
	...
	list_add_tail(&mb_page->node, &area->page_list); /* area arrays need init */
