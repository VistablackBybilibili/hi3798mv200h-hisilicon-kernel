...
#define TEEC_PARAM_TYPE_GET(paramTypes, index) \
	(((unsigned int)(paramTypes) >> (unsigned int)(4*(index))) & (unsigned int)0x0F)
...
static int calc_teecd_path_hash(unsigned char *data, unsigned long len, char *digest)
{
	...
	rc = memset_s(desc, sizeof(struct shash_desc) + crypto_shash_descsize(g_tee_shash_tfm),
			0, sizeof(struct shash_desc) + crypto_shash_descsize(g_tee_shash_tfm));
	if (rc != 0) {
		TCERR("memset desc failed\n");
		kfree(desc);
		return -ENOMEM;
	}
...
static int check_teecd_access(struct task_struct *ca_task)
{
	...
	ret = memset_s(tpath, MAX_PATH_SIZE, 0, MAX_PATH_SIZE);
	if (ret != 0) {
		TCERR("ca_cert memset fail\n");
		kfree(tpath);
		put_cred(cred);
		return -EPERM;
	}
	...
	if (!IS_ERR_OR_NULL(path)) {
		...
		message_size = snprintf(ca_cert, BUF_MAX_SIZE - 1,
				"%s%s%u", ca_task->comm, path,
				cred->uid.val);
#else
		message_size = snprintf(ca_cert, BUF_MAX_SIZE - 1,
				"%s%s%u", ca_task->comm, path,
...
TC_NS_Service *tc_find_service(struct list_head *services, const char *uuid)
{
...
static void tc_notify_timer_fn(struct notify_data_entry *notify_data_entry)
{
	...
	int enc_found = 0;
	rtc_timer_callback_func callback_func = NULL;
	struct TC_NS_Callback *callback_func_t = NULL;
	struct tc_notify_data *tc_notify_data_timer = NULL;
...
{
	struct tp_notify_data_t *tp_nofity_data = NULL;
				 ...
				 TC_NS_ClientContext *context)
{
	...
	* device file pointer */
	...
	context->params[3].memref.size_addr = (__u64)(uintptr_t)&dev_file->pkg_name_len;
	context->params[3].memref.buffer = (__u64)(uintptr_t)dev_file->pkg_name;
	...
	if (dev_file->pub_key_len != 0) {
		...
		context->params[2].memref.buffer = (__u64)(uintptr_t)dev_file->pub_key;
	} else {
...
static int TC_NS_Client_Login(TC_NS_DEV_File *dev_file, const void __user *buffer)
{
	int ret = -EINVAL;
...
#define CERT_BUFFER_SIZE (MAX_PACKAGE_NAME_LEN + MAX_PUBKEY_LEN +\
	...
	sret = memset_s(cert_buffer, CERT_BUFFER_SIZE, 0 , CERT_BUFFER_SIZE);
	if (sret != EOK) {
		TCERR("Failed to memset login buffer!");
		ret = -EINVAL;
		goto error;
	}
...
static int TC_NS_TST_CMD(TC_NS_DEV_File *dev_id, void *argp)
{
	...
	ret = memset_s(&client_context, sizeof(client_context), 0x0, sizeof(client_context));
	if (ret) {
		TCERR("memset &client_context failed\n");
		ret = -EINVAL;
		return ret;
	}
	...
	if (copy_from_user
	    (&cmd_id, (void *)(uintptr_t)client_context.params[0].value.a_addr,
	     sizeof(cmd_id))) {
	...
	switch (cmd_id) {
	case TST_CMD_01:
		callback_demo_main((const char *)client_context.uuid);
		break;
	case TST_CMD_02:
		...
		if (client_context.params[1].value.a_addr == 0 ) {
			TCERR("client_context.params[1].value.a_addr is invalid\n");
			ret = -EFAULT;
			return ret;
		}
		if (copy_to_user
		    ((void *)(uintptr_t)client_context.params[1].value.a_addr, &timer_type,
		     sizeof(timer_type))) {
					  ...
					  void **mb_param, TC_NS_Shared_MEM **used_shm)
{
	...
	if (client_param->memref.size_addr == 0) {
		TCERR("param->memref.size_addr invalid\n");
		ret = -1;
		return ret;
	}
	...
	list_for_each_entry(shared_mem, &dev_file->shared_mem_list, head) {
		if (shared_mem->user_addr ==
			...
			if (shared_mem->len >=
				...
				operation->buffer_h_addr[index] = (virt_to_phys(mb_ptr) >> 16) >> 16; /*lint !e572*/
			}
			break;
		}
	}
...
static int TC_NS_load_image(TC_NS_DEV_File *dev_file, const void *argp, unsigned cmd)
{
	int ret = -1;
	int index = 0;
	...
	struct reg_buf_st  *k_register_buf = NULL;
	struct reg_buf_st  *k_register_buf_p = NULL;
	TC_NS_Shared_MEM *shared_mem = NULL;
	struct mb_cmd_pack *mb_pack = NULL;
	...
	if (client_context.params[0].memref.size_addr == 0) {
		TCERR("client_context.params[0].memref.size_addr is invalid\n");
		ret = -1;
		goto out1;
	}
	...
	if (copy_from_user(&reg_buf_size,
		(void __user *)(uintptr_t)client_context.params[0].memref.size_addr,
		sizeof(unsigned int))) {
	...
	if (ZERO_OR_NULL_PTR((uintptr_t)k_register_buf_p)) {
		TCERR("alloc register buf failed");
		ret = -ENOMEM;
		goto out1;
	}

	if (client_context.params[0].memref.buffer == 0) {
		TCERR("client_context.params[0].memref.buffer is invalid\n");
		ret = -1;
		goto out1;
	}

	if (copy_from_user(k_register_buf_p,
			(void __user *)(uintptr_t)client_context.params[0].memref.buffer,
	...
	for (index = 0; index < reg_buf_size/sizeof(struct reg_buf_st); /*lint !e574 */
		...
		if (k_register_buf->file_buffer == 0) {
			TCERR("share mem get fail\n");
			ret = -1;
			goto out1;
		}
		...
		mb_pack->operation.buffer_h_addr[0] =
			(virt_to_phys((void *)shared_mem->kernel_addr) >> 16) >> 16;
		...
		mb_pack->uuid[0] = 1;
		smc_cmd->uuid_phys = virt_to_phys((void *)mb_pack->uuid);
		smc_cmd->uuid_h_phys = (virt_to_phys((void *)mb_pack->uuid) >> 16) >> 16; /*lint !e572*/
		smc_cmd->dev_file_id = dev_file->dev_file_id;
		smc_cmd->context_id = 0;
		smc_cmd->operation_phys = virt_to_phys(&mb_pack->operation);
		smc_cmd->operation_h_phys = (virt_to_phys(&mb_pack->operation) >> 16) >> 16; /*lint !e572*/
		...
		if (service != NULL) {
			...
			if (!list_empty(&service->session_list)) {
				...
				list_for_each_entry_safe(session, tmp_session,
					...
					mutex_lock(&session->ta_session_lock);
					ret2 = close_session(dev, service->uuid,
						     session->session_id);
					if (TEEC_SUCCESS != ret2)
...
void shared_vma_close(struct vm_area_struct *vma)
{
	...
	void *user_addr = (void *)(uintptr_t)(vma->vm_start);
				    ...
				    unsigned long arg)
{
	...
	ret = memset_s(&context, sizeof(TC_NS_ClientContext), 0x0, sizeof(TC_NS_ClientContext));
	if (ret) {
		TCERR("memset context Failed ret is %d\n", ret);
		ret = -EINVAL;
		return ret;
	}
...
static long tc_agent_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	case TC_NS_CLIENT_IOCTL_WAKE_UP_EVENT: {
		ret = TC_NS_wakeup_event((unsigned int)arg);
		break;
	}
	...
	case TC_NS_CLIENT_IOCTL_SYC_SYS_TIME: {
		ret = TC_NS_sync_sys_time((TC_NS_Time *)(uintptr_t)arg);
		break;
	}
...
static __init int tc_init(void)
{
	...
	if (g_notify_data == NULL) {
		g_notify_data =
			(void *)(uintptr_t)__get_free_page(GFP_KERNEL | __GFP_ZERO);
		if (g_notify_data == NULL) {
