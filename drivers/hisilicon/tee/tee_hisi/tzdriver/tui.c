				  ...
				  size_t cnt, loff_t *ppos)
{
	...
	ret = snprintf(buf, READ_BUF, "tui state:%s\n",
			 state_name[atomic_read(&tui_state)]);
	if (ret < 0)
		return -EINVAL;
	r = (unsigned int)ret;

	ret = snprintf(buf + r, READ_BUF - r, "drv config state:");
	if (ret < 0)
		return -EINVAL;
	r += (unsigned int)ret;
	...
	mutex_lock(&tui_drv_lock);
	list_for_each_entry(pos, &tui_drv_head, list) {
		ret = snprintf(buf + r, READ_BUF - r, "%s-%s,", pos->name,
				1 == pos->state ? "ok" : "no ok");/* buffer足够大不会越界  */
		if (ret < 0) {
			mutex_unlock(&tui_drv_lock);
			return -EINVAL;
		}
				...
				size_t cnt, loff_t *ppos)
{
	...
	ret = snprintf(buf, MSG_BUF, "%s", "event format: event_type:val\n"
			 "event type:\n");
	if (ret < 0)
		return -EINVAL;
	r = (unsigned int)ret;
	...
	for (i = 0; i < TUI_POLL_MAX - 1; i++) {
		ret = snprintf(buf + r, MSG_BUF - r, "%s, ",
				 poll_event_type_name[i]);/* buffer足够大不会越界 */
		if (ret < 0)
			return -EINVAL;
		r += (unsigned int)ret;
	}
	...
	/* cfg drv type list */
	ret = snprintf(buf + r, MSG_BUF - r, "val type for %s or %s:\n",
	...
	mutex_lock(&tui_drv_lock);
	list_for_each_entry(pos, &tui_drv_head, list) {
		ret = snprintf(buf + r, MSG_BUF - r, "%s,", pos->name);
		if (ret < 0) {
			mutex_unlock(&tui_drv_lock);
			return -EINVAL;
		}
