...
int TC_NS_set_nativeCA_hash(unsigned long arg)
{
	...
	operation.params[0].value.b = (unsigned int)((virt_to_phys(buftotee) >> 16) >> 16);
			 ...
			 TC_NS_Shared_MEM *shared_mem)
{
	...
	operation.params[0].value.b = (virt_to_phys(shared_mem->kernel_addr) >> 16) >> 16;
...
int TC_NS_unregister_agent(unsigned int agent_id)
{
	...
	operation.params[0].value.b = (virt_to_phys(event_data->buffer->kernel_addr) >> 16) >> 16;
