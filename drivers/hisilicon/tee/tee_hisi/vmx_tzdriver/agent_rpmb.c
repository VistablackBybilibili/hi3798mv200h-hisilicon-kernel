...
static void get_devinfo(struct tee_agent_kernel_ops *agent_instance)
{
	...
	/* Get EMMC CID */
	ret = snprintf(tmp_cid, sizeof(tmp_cid),
		  "%08x%08x%08x%08x", card->raw_cid[0], card->raw_cid[1],
		  card->raw_cid[2], card->raw_cid[3]);
	if (ret < 0 || ret > sizeof(tmp_cid)) {
		TCDEBUG("snprintf tmp_cid failed.:\n");
	}
	ret = memcpy_s(trans_ctrl->args.get_devinfo.cid, RPMB_EMMC_CID_SIZE, tmp_cid, RPMB_EMMC_CID_SIZE);
	if (ret != 0) {
		TCDEBUG("memcpy tmp_cid failed.:\n");
	}
	/* Get RPMB size */
...
static void dump_memory(uint8_t *data, uint32_t count)
{
	...
	for (i = 0; i < count / 16 ; i++) {
		...
		j = snprintf((char *)buffer, 256, "%x: ", (i * 16));

		if (j < 0)
			break;

		j = snprintf((char *)(buffer + j), 256 - j, "%08x %08x %08x %08x ",
			       *p, *(p + 1), *(p + 2), *(p + 3));
		if (j < 0)
			break;
	...
	if (count % 16) {
		j = snprintf((char *)buffer, 256, "%x: ", ((count / 16) * 16));

		for (i = 0; i < 4; i++) {
			if (j < 0)
				break;

			j += snprintf((char *)(buffer + j), 256 - j, "%08x ", *p++);
...
static int rpmb_agent_work(struct tee_agent_kernel_ops *agent_instance)
{
	...
	if (0 == rpmb_check_data(trans_ctrl)) {
		...
		tlogd("rpmb agent cmd is get_devinfo, agent_instance=%p\n", agent_instance);
#ifdef CONFIG_DEVCHIP_PLATFORM
...
#ifdef RPMB_UPDATE_INIT
/* this function defined but not used */
static int __init rpmb_update_timer_init(void)
{
	rpmb_update_cmd.agent_id = AGENT_RPMB_ID;
