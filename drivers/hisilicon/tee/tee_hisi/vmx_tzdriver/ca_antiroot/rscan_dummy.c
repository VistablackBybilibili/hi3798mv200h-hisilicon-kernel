...
{
	int ret = 0;
	ret = memcpy_s(result->kcode, HASH_DIGEST_SIZE, test_kcode, HASH_DIGEST_SIZE);
	if (ret != 0) {
		pr_err("memcpy kcode failed.\n");
	}
	ret = memcpy_s(result->syscalls, HASH_DIGEST_SIZE, test_syscalls, HASH_DIGEST_SIZE);
	if (ret != 0) {
		pr_err("memcpy syscalls failed.\n");
	}
	ret = memcpy_s(result->sehooks, HASH_DIGEST_SIZE, test_sehooks, HASH_DIGEST_SIZE);
	if (ret != 0) {
		pr_err("memcpy sehooks failed.\n");
	}
		...
		size_t cnt, loff_t *ppos, uint32_t val)
{
	...
	r = memset_s(buf, BUF_SIZE, 0, BUF_SIZE);
	if (r != 0) {
		pr_err("memset failed.\n");
	}
	r = snprintf(buf, BUF_SIZE, "%u\n", val);
		...
		loff_t *ppos, char *array, size_t len)
{
	...
	ret = memset_s(array, MAX_RPROC_SIZE, 0, MAX_RPROC_SIZE);
	if (ret != 0) {
		pr_err("memset array failed.\n");
		kfree(tmp_proc);
		return -EFAULT;
	}
...
{
    struct dentry *result_dentry = NULL;
    struct dentry *status_dentry = NULL;
