...
void tc_mem_free(TC_NS_Shared_MEM *shared_mem)
{
	...
	if (1 == shared_mem->from_mem_pool) {
		...
		free_pages((unsigned long)(uintptr_t)shared_mem->kernel_addr,
...
TC_NS_Shared_MEM *tc_mem_allocate(TC_NS_DEV_File *dev, size_t len)
{
	...
	ret = memset_s(shared_mem, sizeof(TC_NS_Shared_MEM), 0, sizeof(TC_NS_Shared_MEM));
	if (ret != 0) {
		tloge("shared_mem memset failed\n");
		kfree(shared_mem);
		return ERR_PTR(-ENOMEM);
	}
...
void tc_mem_destroy(void)
{
	...
	if (g_mem_pre_allocated) {
		free_pages((uintptr_t)g_mem_pre_allocated, /*lint !e778 !e866 !e747 !e732*/
