		...
		uint32_t cmd_type, uint8_t wait)
{
	...
	register u32 r3 asm("r3") = (cmd_addr >> 16) >> 16;
				  ...
				  uint8_t flags)
{
	...
	ret = memset_s(we, sizeof(struct wait_entry), 0, sizeof(struct wait_entry));
	if (ret) {
		tloge("failed to memset memory!\n");
		kfree(we);
		return (unsigned int)-ENOMEM;
	}
...
int smc_init_data(struct device *class_dev)
{
	...
	cmd_data = (TC_NS_SMC_QUEUE *)(uintptr_t)__get_free_page(GFP_KERNEL | __GFP_ZERO);
	if (cmd_data == NULL)
		return -ENOMEM;
...
free_mem:
	free_page((unsigned long)(uintptr_t)cmd_data);
	cmd_data = NULL;
