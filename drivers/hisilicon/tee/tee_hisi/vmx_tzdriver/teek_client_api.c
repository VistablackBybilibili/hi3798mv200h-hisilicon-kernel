	...
	for (param_cnt = 0; param_cnt < 4; param_cnt++) {
		if ((TEEC_MEMREF_TEMP_INPUT == param_type[param_cnt])
			...
			cli_context->params[param_cnt].memref.buffer =
					(__u64)(uintptr_t)operation->params[param_cnt].tmpref.buffer;
			cli_context->params[param_cnt].memref.size_addr =
				(__u64)(uintptr_t)&operation->params[param_cnt].tmpref.size;
			...
			if (TEEC_MEMREF_WHOLE == param_type[param_cnt]) {
				...
				cli_context->params[param_cnt].memref.size_addr =
					(__u64)(uintptr_t)&operation->params[param_cnt].memref.parent->size;
			} else {
				cli_context->params[param_cnt].memref.offset =
					operation->params[param_cnt].memref.offset;
				cli_context->params[param_cnt].memref.size_addr =
					(__u64)(uintptr_t)&operation->params[param_cnt].memref.size;
			}

			if (operation->params[param_cnt].memref.parent->
			    is_allocated) {
				cli_context->params[param_cnt].memref.buffer =
					(__u64)(uintptr_t)operation->params[param_cnt].memref.parent->buffer;
			} else {
				cli_context->params[param_cnt].memref.buffer =
						(__u64)(uintptr_t)operation->params[param_cnt].memref.parent->buffer +
						operation->params[param_cnt].memref.offset;
			...
			cli_context->params[param_cnt].value.a_addr =
				(__u64 )(uintptr_t)&operation->params[param_cnt].value.a;
			cli_context->params[param_cnt].value.b_addr =
				(__u64 )(uintptr_t)&operation->params[param_cnt].value.b;
		} else if (TEEC_ION_INPUT == param_type[param_cnt]) {
			cli_context->params[param_cnt].value.a_addr
				= (__u64 )(uintptr_t)&operation->params[param_cnt].ionref.ion_share_fd;
			cli_context->params[param_cnt].value.b_addr
				= (__u64)(uintptr_t)&operation->params[param_cnt].ionref.ion_size;
...
static TEEC_Result TEEK_CheckTmpRef(TEEC_TempMemoryReference tmpref)
{
	...
	if ((tmpref.buffer == NULL) || (0 == tmpref.size)) {
...
static TEEC_Result TEEK_CheckMemRef(TEEC_RegisteredMemoryReference memref,
				    uint32_t param_type)
{
	if ((memref.parent == NULL) || (!memref.parent->buffer)) {
