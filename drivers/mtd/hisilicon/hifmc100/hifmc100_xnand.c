				       ...
				       enum dma_data_direction direction)
{
	...
	/* set read command to avoid fmc registers corrupted*/
	reg = &xnand->reg.read;
	hifmc_write(hifmc, reg->cmd, HIFMC100_CMD);
...
{
#if defined(CONFIG_HI3798MV2X_FPGA) || defined(CONFIG_HI3798MV310_FPGA) || defined(CONFIG_HI3796MV2X_FPGA) || defined(CONFIG_HI3716MV430_FPGA)
    return 0;
#else
	u32 regval;
