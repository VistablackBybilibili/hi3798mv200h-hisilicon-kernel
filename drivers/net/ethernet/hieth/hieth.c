...
static void hieth_of_get_phy_trim_params(struct hieth_platdrv_data *pdata, int port_index)
{
	...
	if(get_chipid(_HI3716MV430_MASK) == _HI3716MV430){
		phy_trim_val = readl(ioremap_nocache(REG_BASE_OTP, 0x1000) + REG_OTP_FEPHY_TRIM);
		if (phy_trim_val) {
			ld_am = phy_trim_val & 0x1f;
			ldo_am = (phy_trim_val >> 8) & 0x7;
			r_tuning = (phy_trim_val >> 16) & 0x3f;
		}
		iounmap(ioremap_nocache(REG_BASE_OTP, 0x1000));
	}else{
